...
Understand how Amp's streaming model works and what SQL operations you can use when defining derived tables.
...
## 1. How Amp Applies Your SQL

When a derived table is defined, Amp performs the same cycle for every new block:

1. Read new block from the blockchain
2. Runs your SQL query over the newly arrived rows
3. Append the results to your derived table
4. Repeat for each new block

> Your SQL must work correctly when run incrementally. Operations that require seeing all data at once (like `GROUP BY` with aggregates) don't work in this model.

## 2. Supported Operations

The following SQL operations work in derived table definitions because they can be computed incrementally:
...
### 2.3 Joins with Dependency Tables
...
## 3. Unsupported Operations (and Why)
...
### 4.1 Pre-Filter High-Volume Tables
...
### 4.2 Enrich Events with Block Data
...
### 4.3: Combine Multiple Event Types
...
> If it works in Studio, it will work as a derived table (assuming it follows streaming limitations).
